        ...
        PersitenceManager.retrieveFavorites { [weak self] result in
            ...
            case .success(let favorites):
                if favorites.isEmpty {
                    self.showEmptyStateView(with: "No favorites? Add someone", in: self.view)
                } else {
...
extension FriendListVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        PersitenceManager.updateWith(favorite: favorite, actionType: .remove) { [weak self] error in
            ...
            self.presentAlertControllerOnMainThread(title: "Unable to remove", messageBody: error.rawValue, buttonTitle: "Ok")
